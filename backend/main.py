...
if not GOOGLE_CLIENT_ID:
    print("⚠️ WARNING: GOOGLE_CLIENT_ID is empty! Check your .env file.")
else:
...
async def init_db():
    ...
    try:
        if not MONGODB_URI:
            log.error("❌ MONGODB_URI is empty!")
            return
...
def _run_classical_inference(text: str) -> dict:
    ...
    # Confidence (LinearSVC doesn't provide proba by default)
...
async def google_auth(req: GoogleAuthRequest):
    try:
        ...
        if not GOOGLE_CLIENT_ID:
            log.error("❌ GOOGLE_CLIENT_ID is not configured in backend")
        ...
        log.info(f"👤 Verified Google token for {email}")
        ...
        if not user:
            log.info(f"🆕 Creating new user for {email}")
            ...
            log.warning(f"⚠️ User {email} already exists with provider: {user.get('provider')}")
            # Optional: Allow updating provider or linking accounts
            pass
            
        access_token = create_access_token(data={"sub": email})
        return {"access_token": access_token, "token_type": "bearer"}
    except ValueError as e:
        log.error(f"❌ Google auth verification failed: {e}")
        raise HTTPException(status_code=400, detail="Invalid Google token")
    except HTTPException:
        raise
    except Exception as e:
        import traceback
        log.error(f"🔥 Google auth CRITICAL error: {str(e)}")
...
async def predict_compare(req: PredictRequest, current_user: Optional[dict] = Depends(get_current_user_optional), db: AsyncIOMotorClient = Depends(get_database)):
    ...
    if state.vectorizer and state.lr_model:
        ...
        try:
            ...
            # LinearSVC doesn't have predict_proba; use default 1.0 for confidence
            svm_conf = 1.0
            results.append(CompareResult(
                name="Linear SVM",
                sentiment=mapping.get(pred, pred),
...
async def analyze_voice(file: UploadFile = File(...), current_user: Optional[dict] = Depends(get_current_user_optional), db: AsyncIOMotorClient = Depends(get_database)):
    ...
    try:
        ...
        log.warning("🔈 Speech Recognition: Could not understand audio (UnknownValueError)")
        ...
        import traceback
        log.error(f"🔥 Voice processing CRITICAL error: {str(e)}")
        log.error(traceback.format_exc())
        raise HTTPException(400, f"Error processing audio: {str(e)}")

    if not text.strip():
        log.warning("🔈 Speech Recognition: Recognized text is empty")
